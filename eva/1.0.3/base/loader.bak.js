...
version: 3.2.0
build: 2676
modify: For Eva's Loader Script&CSS, add Yintai.com Combine strategy
changed by idd.chiang
*/
...
// directly or by means of modules it supersedes.
   _requires: function(mod1, mod2) {
    ...
    loadNext: function(mname) {
        ...
        var s, len, i, m, url, fn, msg, attr, group, groupName, j, frag, files, filesQuery, filesURLs,rs,_hash,_u,_pagename,_FORM, 
            comboSource, comboSources, mods, combining, urls, comboBase,_lanchCreateSingleFile,__url,
            // provided,
            type          = this.loadType, 
            self          = this,
		...
		if(this.combine && EVA._Cfg.create){
					if (mname) {
		...
		*/
		...
		for (i=0; i<len; i=i+1) {
			...
			//if (!type || type === m.type) {  online 解封
			if ((EVA._Cfg.create&&EVA._CFG._isCombo) || !type || type === m.type) {
				this._loading = s[i];
				if (m.type === CSS) {
					fn = Y.Get.css;
				...
				/*
				* Combine is false
				* addmojo uses the 'path'
				* by idd.chiang	
				*/
				/*合并时正常显示*/
				//if (!EVA._CFG._isCombo && !m.emojo && typeof m.emojo != 'undefined') {  online 解封
				if(m.emojo && typeof m.emojo != 'undefined'){
					__url = this._filter(m.path, s[i]);
				} else {
					__url = (m.fullpath) ? this._filter(m.fullpath, s[i]) : this._url(m.path, s[i], group.base || m.base);
				}
				__url = __cdn + __url;
				
				/*url = (m.fullpath) ? this._filter(m.fullpath, s[i]) : this._url(m.path, s[i], group.base || m.base);*/
				//console.log(this._onFailure(this.data));
				fn(__url, {
					data:         s[i],
            ...
            for (j in comboSources) {
                if (comboSources.hasOwnProperty(j)) {
                    ...
                    for (i=0; i<len; i++) {
                        ...
                        /*
						* Modify Combo to combine non-yui JS moudels
						* by idd.chiang
						*/
						if ((m && (m.type === type) && (m.combine || !m.ext)) || (m &&
						typeof m.emojo != 'undefined' &&
							m.emojo == true &&
							this.loadType.toLowerCase() == m.path.replace(/^(.+)\.(.+)$/ig, '$2').toLowerCase()
						)) {
							if (typeof m.emojo != 'undefined' && m.emojo == true) {
								var frag = m.path;
							} else {
								//else module is YUI
								var frag = this.root + m.path;
					...
					/*
					* 处理requires URL 列表
					* files [] require URLs Array
					* filesQuery string require URLs' Query
					*/

					if(EVA._GM.onceJS != JS && combining.length){
						 filesQuery = files.join('|');
						 _hash = Eva.MD5(filesQuery);
						
						//Dep.预发布combined URL
						/*
						* 2010/10/14 a.yintai.org 暂无rs重定向
						* 暂时用根目录下basepath测试
						*/
						filesURLs = comboBase + '?file=' + Eva.encodeUri(filesQuery) + '&version=' + EVA._Cfg.release + '&type=' + type + '&hash=' + _hash;
						_pagename = window.location.pathname//.split('/');
						//_pagename = _pagename[_pagename.length-1];

						/*filesURLs = comboBase + '?file=' + Eva.encodeUri(filesQuery) + '&version=' + EVA._Cfg.release + '&type=' + type + '&hash=' + _hash + '&pagename=' + _pagename;*/
						//filesURLs = "file=" + filesQuery + '&version=' + EVA._Cfg.release + '&type=' + type + '&hash=' + _hash + '&pagename=' + _pagename;


						if (EVA._Cfg.create) {
							// 创建IMG 作为预发布时向CDN生成压缩文件
							/*
							因为URL超过2048byte,改用Post提交的方法
							var _$img = new Image();
							_$img.src = filesURLs;
							*/
							_FORM = document.createElement('form');
							_FORM.method = 'POST';
							_FORM.action = 'http://a.yintai.org/default.aspx';//EVA._Cfg.comboBase; //Create 2010年10月29日 生成服务器 a.yintai.org
							_FORM.innerHTML = '<style>.compressor{width:980px;margin:0 auto;}.compressor input,.compressor textarea{width:400px;display:block;padding:3px;margin:3px 0}</style><div class="compressor" id="'+((this.loadType === CSS) ? CSS : JS)+'">'+((this.loadType === CSS) ? '<div class="'+CSS+'"><textarea name="file">'+filesQuery+'</textarea><input type="text" name="version" value="'+EVA._Cfg.release+'"/><input type="text" name="type" value="'+type+'"/><input type="text" name="hash" value="'+_hash+'"/><input type="text" name="pagename" value="'+_pagename+'"/>':'<div class="'+JS+'"><textarea name="file">'+filesQuery+'</textarea><input type="text" name="version" value="'+EVA._Cfg.release+'"/><input type="text" name="type" value="'+type+'"/><input type="text" name="hash" value="'+_hash+'"/><input type="text" name="pagename" value="'+_pagename+'"/>')+'</div></div>';

							var _TA = document.createElement('iframe');
							_TA.name = (this.loadType === CSS) ? 'place'+CSS : 'place'+JS;
							_FORM.target = _TA.name;
							_TA.className = 'hidden';
							
							document.getElementsByTagName('body')[0].appendChild(_TA);
						...
						//向外抛出filesURLs
						EVA._Cfg.combineFilesQuery = filesQuery;
						EVA._Cfg.combineFiles = files;
						EVA._Cfg.combineFilesURLs = filesURLs;
						
						//确保重复加载JS模块
						EVA._GM.onceJS = this.loadType;

						//URLs For page
						//区分JS与CSS
						/*
						if (this.loadType === CSS) {
							_u = rs + CSS + '/' + EVA._Cfg.release + '/' + _hash + '.' + CSS;
						} else {
							_u = rs + JS + '/' + EVA._Cfg.release + '/' + _hash + '.' + JS;
						}
						*/
						//2010/10/20 EVA._CFG.comboDomain  合并只在a.yintai.org上完成						
						if (this.loadType === CSS) {
							_u = EVA.cdn + rs + CSS + '/' + _hash + '.' + CSS + '?version='+EVA._Cfg.release;
						} else {
							_u = EVA.cdn + rs + JS + '/' + _hash + '.' + JS + '?version='+EVA._Cfg.release;
						}
						if(!EVA._Cfg.create && this.combine){
							urls.push(this._filter(_u));
						}
	  ...
	  // console.log(this);
	   for (i=0; i<len; i=i+1) {
			...
			// the css separately from the script.
			//if (!type || type === m.type) {  online 解封
			if (EVA._Cfg.create || !type || type === m.type) {
				this._loading = s[i];
				if (m.type === CSS) {
					fn = Y.Get.css;
					attr = this.cssAttributes;
				...
				*/
				...
				//if (!EVA._CFG._isCombo && !m.emojo && typeof m.emojo != 'undefined') {  online 解封
				if(EVA._Cfg.create && this.combine){
					this._u = EVA._Cfg.cdn;
					if(m.emojo && typeof m.emojo != 'undefined'){
						url = this._filter(m.path, s[i]);
					} else {
						//debugger;
						url = (m.fullpath) ? this._filter(m.fullpath, s[i]) : this._url(m.path, s[i], group.base || m.base);
					}
					...
					this._u = '';
					if(!EVA._CFG._isCombo && !m.emojo && typeof m.emojo != 'undefined'){
						url = this._filter(m.path, s[i]);
					} else {
						//debugger;
						url = (m.fullpath) ? this._filter(m.fullpath, s[i]) : this._url(m.path, s[i], group.base || m.base);
