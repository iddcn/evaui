 ...
 * @constructor EVA
 */	
var EVA = EVA || function(){
	this.init.apply(this, arguments);
};

//EVA configuration
if(typeof evaCfg == 'undefined' || typeof evaCfg === null)var evaCfg={};
evaCfg._scripts = document.getElementsByTagName("script");
	/*
	*	YinTai 获取参数方式
	*	@evaCfg 配置
	*	@param {boolen} combine 是否合并 为空的时候默认为true
	*	@param {boolen} create 是否向CDN生成POST请求 默认为false
	*/
	evaCfg._src = evaCfg._scripts[evaCfg._scripts.length - 1].src;
	eval(evaCfg._scripts[evaCfg._scripts.length - 1].innerHTML);
	if(evaCfg._src.indexOf('#') > -1 && evaCfg._src.indexOf('?') > -1){
		var variable = evaCfg._src.replace(/#/i,'&').split('?')[1];
	}else{
		var variable = evaCfg._src.split('?')[1];
	}
	if(typeof variable !== 'undefined'){
		variable = variable.split('&');
		for(var i = 0, m = variable.length; i<m; i++){
			evaCfg[variable[i].split('=')[0]] = variable[i].split("=")[1];
		}
	}
	
EVA.cdn = (typeof evaCfg.cdn !== 'undefined') ? evaCfg.cdn : 'http://a.yintai.org/';

EVA._Cfg = {
	ver:'1.0.3',
	release: (typeof evaCfg.version !== 'undefined') ? evaCfg.version : '20101115',
	combo: (typeof evaCfg.combine !== 'undefined') ? ((evaCfg.combine === 'true') ? true : false) : false,
	create:(typeof evaCfg.create !== 'undefined') ? ((evaCfg.create === 'true') ? true : false) : false,
	path:(typeof evaCfg.path !== 'undefined') ? evaCfg.path : window.location.protocol=='https:'?'/rs/':'rs/',
	//资源文件存放域
	cdn:EVA.cdn,
	//comboBase:(typeof evaCfg.comboBase !== 'undefined') ? evaCfg.comboBase : EVA.cdn + 'default.aspx',
	comboBase:'http://a.yintai.org/default.aspx', //生成链接只向a.yintai.org中
	/*create : 用于简化Combine后URL是否向页面中生成img标签，默认是true*/
	DOC_LABEL:'eva-enabled'
};


EVA._CFG = {};//全局配置
	EVA._CFG._isCombo = EVA._Cfg.combo;
	EVA._CFG.basepath = EVA._CFG._isCombo ? EVA._Cfg.path : EVA._Cfg.cdn + EVA._Cfg.path;
	EVA._CFG.evaBase = EVA._CFG.basepath + 'js/eva/' + EVA._Cfg.ver + '/';
	EVA._CFG._isRelease = EVA._CFG._isCombo ? '':'?version='+EVA._Cfg.release;
	//仅使用eva.combo.js做合并，获取文件自a.yintai.org
	EVA._CFG.comboDomain = EVA._Cfg.cdn + EVA._Cfg.path + 'js/eva/' + EVA._Cfg.ver + '/';
...
(function() {
    var proto, prop,
        VERSION       = EVA._Cfg.ver||'3.2.0', 
        BASE          = 'http://yui.yahooapis.com/',
        DOC_LABEL     = EVA._Cfg.DOC_LABEL||'yui3-js-enabled',
        NOOP          = function() {},
    ...
    use: function() {
        ...
        if (boot && len && Y.Loader) {
            ...
            if (G_ENV._bootstrapping) {
                ...
                Y.Get.script(EVA._CFG.comboDomain+'base/loader.js' + '?version='+EVA._Cfg.release, {	//config.base + config.loaderPath
                    onEnd: handleBoot 
                });
            }
...
Y.Get = function() {
    ...
    _fail = function(id, msg) {
		if(typeof this._K !== 'undefined'){
			document.getElementsByTagName('head')[0].removeChild(this._K.nodes[0]);
		};
        var q = queues[id], sc;
		//console.log(q.data);
        if (q.timer) {
...
EVA._GM = {};//global Modules
EVA._GR = [];//global Requires
EVA._AGR = [];//all global Requires
EVA._INFO = {
	path:EVA._CFG.evaBase+'base/info.js',
...
EVA.prototype = {
	init:function(){
		//构造
		this.evaInfo = EVA._INFO;//EVAInfo
		this.modules = EVA._GM;
		this.addedMojo = [];
		this.requiredMojo = EVA._GR;
		this.allRequiredMojo = EVA._AGR;
	...
	addmojo:function(modules){
		...
		fd.modules.info=fd.modules.EVA||fd.evaInfo;
	...
	*/
	...
	/**
	* encodeUri
	* @param { value } o : enCodeURL
	* @member EVA
	*/
	encodeUri : function(o) {
		return this.rfcEncodeUri(o);
		//o = typeof o === "string" ? o : this.parse(o);
	},
	rfcEncodeUri: function(o) {
		o = typeof o === "string" ? o : this.parse(o);
		return encodeURIComponent(o)
			.replace( /\+/g,	"%2B" )
			.replace( /%20/g,	"+" );
	},
	decodeUri : function(o) {
		return this.rfcDecodeUri(o);
		o = typeof o === "string" ? o : this.parse(o);
		return unescape(o);
	},
	rfcDecodeUri : function(o) {
		o = typeof o === "string" ? o : this.parse(o);
...
var onDOMContentLoaded = function(onready,config){  
    ...
    if(config)  
    for(var p in config)this.conf[p] = config[p];  
    var isReady = false;  
    function doReady(){  
		if( isReady ) return;  
        isReady = true;  
...
var S = S || YUI();
/*
var yuiCfg =  typeof YUI_CONFIG !== 'undefined'?YUI_CONFIG : {
	combine:true,
...
*/
EVA.yuiCfg = {
	base:EVA._CFG.basepath+'js/yui/3.2.0/build/',
	charset:'utf-8',
	root:'rs/js/yui/3.2.0/build/',
	comboBase:EVA._Cfg.comboBase, 
	rs:EVA._CFG.basepath, 
	combine:EVA._CFG._isCombo,
	timeout:3000,
	loadOptional: true,
    throwFail:(EVA._Cfg.combo && EVA._Cfg.create)
};
var Eva = new EVA().addmojo();

onDOMContentLoaded(function(){
	S.Get.script(EVA._CFG.comboDomain+'base/mojo.debug.js?version='+EVA._Cfg.release, {
		onSuccess: function() {
			S.mix(EVA.yuiCfg,{modules:Eva.modules});
			YUI(EVA.yuiCfg).use('info',function(Y){
				Y.mix(S,Y);
				for(var i = 0;i< Eva.onReadys.length;i++){
					Eva.onReadys[i].apply(this,[Y,Eva]);
				}
			});
		}
	});
