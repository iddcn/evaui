...
YUI.add('box', function (Y) {
    ...
    Y.Box.prototype = {
        ...
        */
        init: function (opt) {
            ...
            that.overlay._posNode.on('focus', function (e) {
                //处理input焦点问题
                var el = e.target;
                //if(el.get('tagName')!=='INPUT'||el.get('tagName')!=='TEXTAREA')el.blur();
            });
        ...
        */
        addMask: function () {
            ...
            //zuoxianjin changes at 20101022 "set mask node's height to document.body.docHeight"
    ...
    */
    Y.Box.alert = function (msg, callback, opt) {
        ...
        var closeText = (typeof opt.closeText == 'undefined' || opt.closeText == null) ? '<i class="shutdown" title="\u5173\u95ED">\u5173\u95ED</i>' : opt.closeText;
        var btnText = (typeof opt.btnText == 'undefined' || opt.btnText == null) ? '\u786E\u5B9A' : opt.btnText;
        var btnAlign = (typeof opt.btnAlign == 'undefined' || opt.btnAlign == null) ? 'center' : opt.btnAlign; //对齐方式

        var closestr = closeable ? '<a class="close closebtn">' + closeText + '</a>' : '';
        var headstr = '<span class="title">' + title + '</span>' + closestr;
        opt.head = headstr;
        opt.body = msg;
        opt.foot = '<div align="' + btnAlign + '"><button class="okbtn c-btn c-btn-small">' + btnText + '</div>';
    ...
    */
    Y.Box.confirm = function (msg, callback, opt) {
        ...
        var canclestr = cancleBtn ? '<button class="canclebtn c-btn c-btn-small">' + cancleText + '</button>' : '';
        var closestr = closeable ? '<a class="close closebtn">' + closeText + '</a>' : '';
        var headstr = '<span class="title">' + title + '</span>' + closestr;
        opt.head = headstr;
        opt.body = msg;
        opt.foot = '<div align="' + btnAlign + '" class="btnmargin"><button class="yesbtn c-btn c-btn-small">' + yesText + '</button><button class="nobtn c-btn c-btn-small">' + noText + '</button>' + canclestr + '</div>';
        opt.onload = function (box) {
            var node = box.overlay._posNode;
