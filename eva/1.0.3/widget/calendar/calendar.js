            ...
            */
            this.buildEvent = function () {
                ...
                cc.EV[0] = con.query('div.dbd').on('click', function (e) {
                    ...
                    var now = new Date();
                    now.setYear(cc.year); //此处应该先设定年月，再设定日，这样可以很好的利用日期溢出效果。
                    now.setMonth(cc.month);
                    now.setDate(selectedd);
                    //that.callback(d);
                    //datetime的date
                    cc.fathor.dt_date = now;
                    cc.fathor.EventCenter.fire('select', now);
                    if (cc.fathor.popup && cc.fathor.closeable) {
                        cc.fathor.hide();
                    }
                    if (cc.fathor.range_select) {
                        cc.fathor.handleRange(now);
                    }
                    cc.fathor.render({ selected: now });
                });
                //向前
                ...
                if (cc.fathor.navigator) {
                    ...
                    cc.EV[4] = con.query('.setime').on('click', function (e) {
                        ...
                        if (e.target.hasClass('ok')) {
                            ...
                            cc.fathor.EventCenter.fire('switch', new Date(_year + '/' + _month + '/01'));
                            cc.fathor.con.query(".setime").addClass("hidden");
                            /*
                            此处如果直接使用con，在IE中会出现错误，跟踪发现在此处的con是用过ID引用到的，ID是一个随机数，
                            所以render前和render后引用到的实例不是同一个，奇怪的是在FF中，两次获取到的ID竟然一样，故在FF中没有该错误提示。
                            */
                            //                            con.query('.setime').addClass('hidden');
