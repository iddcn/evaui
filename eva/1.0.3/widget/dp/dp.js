...
// opens a new windows and execute the original unformatted source code inside.
dp.sh.Utils.ExeSource = function(sender)
{
	var code = sender.parentNode.originalCode;
...
// show the textarea and edit the original unformatted source code inside.
dp.sh.Utils.EditSource = function(sender)
{
	var el = new S.Node(sender);
	var con = el.ancestor('.dp-highlighter');
	try{
		var tb = con.getElementsByTagName('table')[0];
		var tbd = tb.getElementsByTagName('tbody')[0];
		tb.removeChild(tbd);
	} catch(o) {}
	var txa = con.next();
	//debugger;
	if(txa.getStyle('height') != '0px'){ //收起
		txa.setStyles({
			'visibility':'',
			'display':'none',
			'height':'0px'
		});
		S.all('.dp-highlighter').each(function(node){
			node.remove();
...
dp.sh.Highlighter.prototype.SwitchToTable = function()
{
	...
	{
		return '<a href="javascript:void(0);" onclick="dp.sh.Utils.' + util + '(this); return false;">' + text + '</a>';
	}
	...
	if(this.addControls == true)
	{
		...
		cell.innerHTML += UtilHref('ViewSource', dp.sh.Strings.ViewPlain) + pipe /*+ UtilHref('PrintSource', dp.sh.Strings.Print)+ pipe*/ + UtilHref('ExeSource', dp.sh.Strings.ExeSource) +  pipe + UtilHref('EditSource', dp.sh.Strings.EditSource);
...
dp.init = dp.SyntaxHighlighter.HighlightAll;
